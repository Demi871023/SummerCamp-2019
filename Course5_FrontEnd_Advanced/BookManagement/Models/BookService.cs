...
namespace BookManagement.Models
{
    public class BookService
    {
        ...
        /// </summary>
        /// <returns>圖書編號</returns>
        public bool UpdateBookData(Models.Books books)
        {
            bool sucess = true;
        ...
        public List<Models.BookRecord> GetBookLendRecord(int BookID)
        {
            ...
            string sql = @"SELECT	
												...
												mm.[USER_ID] AS LendUserId, 
												mm.USER_CNAME AS LendUserCName, 
												mm.USER_ENAME AS LendUserEName,
												blr.BOOK_ID
        ...
        private List<Models.BookRecord> MapBookDataToListForLendRecord(DataTable BookData)
        {
            ...
            foreach (DataRow row in BookData.Rows)
            {
                result.Add(new BookRecord()
                {
					...
					LendUserId = row["LendUserId"].ToString(),
					LendUserCName = row["LendUserCName"].ToString(),
					LendUserEName = row["LendUserEName"].ToString()
				});
            }
            return result;
        }
